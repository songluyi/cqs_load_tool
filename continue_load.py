...
def continue_load_db():
    ...
    sql_item='update cux.cux_cqs_items_his_t set batch_id='+str(change_batch_id)+'where batch_id='+str(get_batch_id())
    sql_pt_rating='update cux.cux_cqs_pt_rating_his_t set batch_id='+str(change_batch_id)+'where batch_id='+str(get_batch_id())
    sql_thickness='update cux.cux_cqs_pipe_thickness_his_t set batch_id='+str(change_batch_id)+'where batch_id='+str(get_batch_id())
    sql_connect='update CUX.CUX_CQS_BRANCH_CONNECT_HIS_T set batch_id='+str(change_batch_id)+'where batch_id='+str(get_batch_id())
    sql_note='update cux.cux_cqs_notes_his_t set batch_id='+str(change_batch_id)+'where batch_id='+str(get_batch_id())
    ...
    sql_batch='delete cux.cux_cqs_batchs_t where batch_id='+str(get_batch_id()+1)
    r=cur.execute(sql_index)
    r=cur.execute(sql_item)
    r=cur.execute(sql_pt_rating)
